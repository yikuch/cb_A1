...
void stackPush(IntStack *self, int i){
	...
	if((new = malloc(sizeof(StackStruct))) != NULL){
		new->data = i;
		new->next = self->top; //next zeigt immer auf das vorher hinzugefügte 
		self->top = new; // top zeigt immer auf das am neusten hinzugefügte
		count++;
	}else{
...
int stackPop(IntStack *self){
	...
	if(count > 0){
		StackStruct *temp = self->top; //wofuer temp?,  wird hier nicht verwendet
